...
// import { handleValidateSelectInputRm } from "@/utils/functions"

function SelectInputRm({
    id,
    label,
    value = [],
    setValue,
    ...
    const handleValidateSelectInputRm = (length, isError, setError, setValidateRows, rowId) => {
        if (isError) {
    ...
    const handleRemoveItem = (v) => {
        ...
        handleValidateSelectInputRm(newValue, isError, setError, setValidateRows, id)
    }
    ...
    const handleBlur = () => {
        setFocus(false)
        handleValidateSelectInputRm(value, isError, setError, setValidateRows, id)
    }
    ...
    return (
        <>
            <div
                ref={containerRef}
                data-component="SelectInputRm"
                className={cl(
                    "relative min-h-[44px] cursor-pointer border-b-2 bg-neutron-4 pb-1",
                    {
            ...
            >
                {/* Bỏ phần label phía trên */}
                <div className="flex min-h-[34px] items-end pt-1">
                    <div className="ml-0 flex w-fit min-w-[0px] max-w-[300px] flex-wrap">
                        {list
                    ...
                    {canSearch && (
                        <input
                            ...
                            placeholder={label} // Dùng label làm placeholder
                            onFocus={handleFocus}
                            onBlur={handleBlur}
                ...
                <i className="absolute right-3 bottom-1">
                    <MdOutlineKeyboardArrowDown
                        className={cl("text-2xl transition-all", { "rotate-180": focus })}
                    />
                </i>
                <div
                    className={cl(
                        "absolute left-0 min-w-[100px] origin-top-left rounded-l",
